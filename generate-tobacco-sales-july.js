...
async function generateTobaccoSalesJuly() {
  ...
  try {
    ...
    console.log("\n🚬 Generating July 2025 tobacco sales targeting $11,900...");
    ...
    let currentTransactionId = 71001; // Start from clean range for tobacco
    ...
    // Target and tracking
    const targetAmount = 11900;
    let totalGenerated = 0;
    const transactions = [];

    // Payment method distribution (same as other months)
    ...
    console.log(`📅 Generating tobacco transactions for July 1-31, 2025...`);
    ...
    while (totalGenerated < targetAmount) {
      ...
      const timestamp = new Date(2025, 6, day, hour, minute); // Month is 0-indexed
      ...
      for (let i = 0; i < itemCount; i++) {
        ...
        // Quantity logic: higher chance for singles, lower for multiples
        let quantity;
      ...
      if (transactions.length > 1000) {
        console.log("⚠️  Reached 1000 transactions, stopping generation");
        break;
      }
    }
    ...
    if (transactions.length > 0) {
      ...
      console.log(`\n✅ Verification:`);
      console.log(`   July tobacco transactions in DB: ${verifyCount}`);
      console.log(
        `   Total July tobacco sales: $${verifyTotal[0]?.total.toFixed(2) || 0}`
      );

      // Final summary with all categories for July
      console.log(`\n🏁 July 2025 Sales Summary:`);

      // Get all July sales by category
      ...
      console.log(`\n🎉 July 2025 tobacco sales generation complete!`);
    } else {
      console.log("❌ No transactions generated");
    }
  } catch (error) {
    console.error("Error generating tobacco sales:", error);
  } finally {
    await client.close();
  }
}

generateTobaccoSalesJuly();
