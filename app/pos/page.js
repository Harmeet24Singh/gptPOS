...
import { updateInventoryAfterTransaction, updateItemStockLocal, loadInventory } from "../lib/slices/inventorySlice";
import {
  Container,
  POSGrid,
  ProductsPanel,
...
function POSContent() {
  ...
  const { items: inventory, loading: inventoryLoading, categories, getFilteredInventory } = useInventoryManager();
  ...
  // Credit card charge state
  const [creditCardCharge, setCreditCardCharge] = useState(false);
      ...
      setCart(
        cart.map((item) => {
          ...
          const isMatch = product.isManual ? 
            (item.isManual && item.name === product.name && item.price === product.price && item.category === product.category) :
            (item.id === product.id);
  ...
  const isCategoryTaxable = (category) => {
    ...
    const taxableKeywords = ['taxable', 'lotto', 'lottery', 'tobacco', 'alcohol'];
    const nonTaxableKeywords = ['non-taxable', 'untaxable', 'basic', 'essential'];
  ...
  const calculateTotal = () => {
    ...
    const ccCharge = creditCardCharge ? 0.25 : 0; // Add credit card charge if selected
    return Math.max(0, subtotal - discount + cbFee + ccCharge); // Ensure total doesn't go negative
  };
    ...
    try {
      ...
      if (transactionResponse.ok) {
        ...
        setExistingCustomers(prev => 
          prev.map(c => c.customerName === selectedCustomer.customerName 
        ...
        console.log(`Payment processed: $${paymentTotal.toFixed(2)} paid, remaining balance: $${remainingBalance.toFixed(2)}`);
      } else {
  ...
  const handleTerminalPayment = async (amount) => {
    try {
      ...
      if (terminalResult.success) {
        ...
        alert(`✅ Payment Approved!\n\nTransaction ID: ${terminalResult.transactionId}\nCard: ${terminalResult.cardType} ****${terminalResult.last4}\nAmount: $${amount.toFixed(2)}\n\nProcessing sale...`);
  ...
  const processCheckout = () => {
    ...
    const discount = Number(parseFloat(discountAmount) || 0);
    const subtotal = Math.max(0, originalSubtotal - discount); // Discounted subtotal
    ...
    // Note: credit card charge is already included in total from calculateTotal()
    const ccCharge = creditCardCharge ? 0.25 : 0;
    ...
    if (isCreditSale) {
      ...
      if (specifiedCreditAmount > 0) {
        ...
        if (requiredPayment > paidTotal + 0.001) {
          setPaymentError(`Need $${requiredPayment.toFixed(2)} payment for $${creditBalance.toFixed(2)} credit (Currently paid: $${paidTotal.toFixed(2)})`);
          return;
        }
      ...
      if (cashback > 0) {
        if (paidCard < totalWithCashback - 0.001) {
          setPaymentError(`Cashback requires card payment of $${totalWithCashback.toFixed(2)} (Total with Cashback: $${totalWithCashback.toFixed(2)})`);
          return;
        }
      } else {
        // Regular transaction without cashback
        if (paidTotal < totalWithCashback - 0.001) {
          setPaymentError(`Payment is insufficient. Need $${totalWithCashback.toFixed(2)} for purchase`);
          return;
        }
      }
    ...
    if (isCreditSale) {
      ...
      if (isPartialPayment) {
        ...
        paymentBreakdown.push({ method: "credit", amount: +creditBalance.toFixed(2), customerName: creditCustomerName.trim() });
      } else {
        // Full credit sale (no payment)
        paymentBreakdown.push({ method: "credit", amount: +totalWithCashback.toFixed(2), customerName: creditCustomerName.trim() });
      }
    } else {
      if (finalCashAmount !== 0)
        paymentBreakdown.push({ method: "cash", amount: +finalCashAmount.toFixed(2) });
      if (finalCardAmount > 0)
        paymentBreakdown.push({ method: "card", amount: +finalCardAmount.toFixed(2) });
    }
    
    // Add lotto winnings to payment breakdown for tracking
    if (lottoAmount > 0)
      paymentBreakdown.push({ method: "lotto", amount: +lottoAmount.toFixed(2) });
    ...
    // Add credit card charge as a line item if present
    const ccChargeForLineItem = creditCardCharge ? 0.25 : 0;
    if (ccChargeForLineItem > 0) {
      transactionItems.push({
        id: 'credit-card-fee-item',
        name: 'Credit Card Fee',
        quantity: 1,
        price: ccChargeForLineItem,
        applyTax: false, // Credit card processing fees are typically not taxable
        category: 'Fees',
      });
    }
    ...
    const newTotal = total + cashback - lottoAmount + ccCharge;
    ...
    const transaction = {
      ...
      creditCardCharge: creditCardCharge ? 0.25 : 0
    };
    ...
    fetch("/api/transaction", {
      ...
      .then(async (data) => {
        if (data && data.transaction) {
          ...
          setCreditAmount(0);
          setPaymentError("");
          setCreditCardCharge(false); // Reset credit card charge
  ...
  const handleCustomerNameChange = (value) => {
    ...
    if (value.length > 0 && Array.isArray(existingCustomers)) {
      const matchingCustomers = existingCustomers.filter(customer =>
        customer && customer.customerName && customer.customerName.toLowerCase().includes(value.toLowerCase())
      );
      ...
      const exactMatch = existingCustomers.find(customer =>
        customer && customer.customerName && customer.customerName.toLowerCase() === value.toLowerCase()
      );
      if (exactMatch) {
  ...
  const processCustomerPayment = async (paymentAmount) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Update the customers list
        setExistingCustomers(prev => 
          prev.map(c => c.customerName === updatedCustomer.customerName ? updatedCustomer : c)
        );
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    if (creditPaymentParam && existingCustomers.length > 0) {
      const targetCustomer = existingCustomers.find(
        customer => customer.customerName.toLowerCase() === creditPaymentParam.toLowerCase()
      );
  ...
  const printReceipt = () => {
    ...
    const receiptContent = `
      ...
      <html>
        <head>
          <title>Receipt - Transaction #${(lastTransaction.id || 'N/A').toString().slice(-8)}</title>
          <style>
            body {
        ...
        <body>
          <div class="header">
            ...
            <div>Transaction #${(lastTransaction.id || 'N/A').toString().slice(-8)}</div>
          ...
          <div class="receipt-totals">
            ${lastTransaction.taxableAmount > 0 && lastTransaction.nonTaxableAmount > 0 ? `
              <div class="receipt-item">
            ...
            ${(lastTransaction.creditCardCharge && lastTransaction.creditCardCharge > 0) ? `
              <div class="receipt-item" style="color: #6f42c1;">
                <span>Credit Card Fee:</span>
                <span>$${lastTransaction.creditCardCharge.toFixed(2)}</span>
              </div>
            ` : ''}
            ...
            ${lastTransaction.paymentBreakdown && lastTransaction.paymentBreakdown.length > 0 ? `
              <hr>
              <div style="font-weight: bold; margin-bottom: 5px;">Payments</div>
              ${lastTransaction.paymentBreakdown.map(p => `
                <div class="receipt-item">
  ...
  useEffect(() => {
    ...
    setPaymentError("");
  }, [cart, cashbackAmount, cashbackFee, lottoWinnings, discountAmount, isCreditSale, creditCardCharge]);
  ...
  }, [autoFillOther, lastEdited, cashAmount, cardAmount, cart, cashbackAmount, cashbackFee, lottoWinnings, discountAmount, isCreditSale, creditCardCharge]);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event) => {
      ...
      if (document.activeElement !== searchInputRef.current && 
          !isTypingInInput &&
          !['Tab', 'Escape', 'F1', 'F2', 'F3', 'F4', 'F5', 'F6', 'F7', 'F8', 'F9', 'F10', 'F11', 'F12'].includes(event.key)) {
        searchInputRef.current?.focus();
      }
    ...
    const handleFocusAnywhere = (event) => {
      ...
      const isCategoryFilter = clickedElement.closest('select') || 
  ...
  const handleBarcodeScanned = (barcode) => {
    ...
    if (searchResults.length === 1) {
      ...
      console.log(`Multiple products found for barcode ${barcode}:`, searchResults.map(p => p.name));
    } else {
      ...
      <Container>
        <ReceiptSection>
          ...
          <div className="receipt">
            ...
            <p>Transaction #{(lastTransaction.id || 'N/A').toString().slice(-8)}</p>
            ...
            <div className="receipt-totals">
              ...
              {lastTransaction.discount > 0 ? (
                <>
                  <div>Items Subtotal: ${lastTransaction.originalSubtotal.toFixed(2)}</div>
              ...
              {lastTransaction.creditCardCharge && lastTransaction.creditCardCharge > 0 && (
                <div style={{ color: "#6f42c1" }}>
                  Credit Card Fee: ${lastTransaction.creditCardCharge.toFixed(2)}
                </div>
              )}
              {lastTransaction.includeTax && lastTransaction.tax > 0 && (
          ...
          <div style={{ display: "flex", gap: "1rem", marginTop: "2rem" }}>
            <Button onClick={printReceipt} style={{ backgroundColor: "#27ae60", color: "white" }}>
              🖨️ Print Receipt
            </Button>
      ...
      <POSGrid>
        <ProductsPanel>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
            <h2>Products</h2>
            <div style={{ fontSize: "0.75rem", color: "#7f8c8d", padding: "0.25rem 0.5rem", background: "#f8f9fa", borderRadius: "4px" }}>
              {inventory.length} items loaded
              {inventoryLoaded && (
                <span style={{ marginLeft: "0.5rem", color: inventoryLoading ? "#f39c12" : "#27ae60" }}>
          ...
          <div style={{
            ...
            {[
              { name: 'Lotto', key: 'Lotto', icon: '🎲', color: '#e74c3c', description: 'Type amount & click to add' },
              { name: 'Grocery', key: 'Grocery', icon: '🛒', color: '#27ae60', description: 'Type amount & click to add' },
              { name: 'Western Union', key: 'WesternUnion', icon: '💸', color: '#8e44ad', description: 'Type amount & click to add' },
              { name: 'Tea', key: 'Tea', icon: '🍵', color: '#d35400', description: 'Type amount & click to add' },
              { name: 'Bakery', key: 'Bakery', icon: '🍞', color: '#f39c12', description: 'Bread & Baked Goods' },
              { name: 'Dairy', key: 'Dairy', icon: '🥛', color: '#16a085', description: 'Milk & Dairy Products' },
              { name: 'Beverages', key: 'Beverages', icon: '🥤', color: '#3498db', description: 'Drinks & Refreshments' },
              { name: 'Tobacco', key: 'Tobacco', icon: '🚬', color: '#95a5a6', description: 'Cigarettes & Cigars' },
              { name: 'Snacks', key: 'Snacks', icon: '🍿', color: '#e67e22', description: 'Chips & Snacks' }
            ].map((category) => (
              <div
                key={category.key}
                ...
                onClick={() => {
                  ...
                  if ((category.key === 'Lotto' || category.key === 'Grocery' || category.key === 'WesternUnion' || category.key === 'Tea') && searchTerm.trim()) {
                    ...
                    if (!isNaN(numericValue) && numericValue > 0) {
                      ...
                      const currentCard = document.querySelector(`[data-category="${category.key}"]`);
                      if (currentCard) {
                ...
                style={{
                  ...
                  boxShadow: categoryFilter === category.key 
                    ? `0 8px 25px ${category.color}40` 
                    : '0 4px 15px rgba(0,0,0,0.1)',
                  transform: categoryFilter === category.key ? 'translateY(-2px)' : 'translateY(0)',
                  border: categoryFilter === category.key ? `3px solid ${category.color}` : '3px solid transparent'
                }}
                onMouseEnter={(e) => {
          ...
          {showManualEntry && (
            <div style={{
              ...
              >
                <h3 style={{ marginBottom: '1rem', color: '#2c3e50' }}>Add Manual Item</h3>
                ...
                <div style={{ marginBottom: '1rem' }}>
                  ...
                  >
                    {categories.length > 0 ? (
                      ...
                      <>
                        <option value="grocery-taxable">Grocery (Taxable)</option>
                        <option value="grocery-non-taxable">Grocery (Non-Taxable)</option>
          ...
          <div className="products-grid">
            {filteredInventory.length === 0 && inventory.length > 0 && searchTerm && (
              <div style={{ padding: '2rem', textAlign: 'center', color: '#f39c12' }}>
                <h3>No products match your search</h3>
                <p>Search: "{searchTerm}" {categoryFilter && `| Category: "${categoryFilter}"`}</p>
                <div style={{ 
                  marginTop: '1rem', 
                  padding: '0.75rem', 
                  ...
                  💡 <strong>Press Enter</strong> to add "{searchTerm}" as a manual item with custom price
                </div>
              </div>
            )}
            {inventoryLoading && (
              <div style={{ padding: '2rem', textAlign: 'center', color: '#3498db' }}>
            ...
            {inventory.length === 0 && !inventoryLoading && (
              <div style={{ padding: '2rem', textAlign: 'center', color: '#e74c3c' }}>
                ...
                <p>No inventory items found. Please check your connection or contact support.</p>
              </div>
            )}
            {filteredInventory.map((product) => (
              ...
              >
                ...
                <div className="stock" style={{
                  ...
                  Stock: {product.stock} {product.stock < 0 ? '(Negative Stock)' : product.stock === 0 ? '(Out of Stock)' : product.stock <= product.lowStockThreshold ? '(Low Stock)' : ''}
                </div>
              </ProductItem>
            ))}
          </div>
                ...
                <div style={{
                  ...
                  {selectedCustomer && (
                    <span style={{ marginLeft: "0.5rem" }}>
                      (Balance: ${Math.abs(customerBalance).toFixed(2)} {customerBalance > 0 ? "owed" : "credit"})
                    </span>
                  )}
                </div>
              )}
            ...
            {cashbackEnabled && (
              <>
                <div style={{ 
                  ...
                  <label style={{ minWidth: "70px", fontSize: "0.9rem" }}>Amount:</label>
                  <input
                    type="number"
                    step="0.01"
                    min="0"
                ...
                <div style={{ 
                  ...
                  <label style={{ minWidth: "70px", fontSize: "0.9rem", color: "#e67e22" }}>Fee:</label>
                  <input
                    type="number"
                    step="0.01"
                    min="0"
                ...
                {(Number(cashbackFee) || 0) > 0 && (
                  <div style={{
                    ...
                    💰 Cashback fee: ${(Number(cashbackFee) || 0).toFixed(2)} added to total
                  </div>
                )}
              </>
            )}
          ...
          <div style={{
            ...
            <div style={{
              ...
              <label style={{ 
                ...
                <input
                  type="checkbox"
                  checked={creditCardCharge}
                  onChange={(e) => {
                    const isChecked = e.target.checked;
                    setCreditCardCharge(isChecked);
                    
                    const currentCardAmount = Number(parseFloat(cardAmount) || 0);
                    const currentCashAmount = Number(parseFloat(cashAmount) || 0);
                    
                    if (isChecked) {
                      // Move all cash to card + add $0.25 credit card fee
                      const totalAmount = currentCardAmount + currentCashAmount + 0.25;
                      setCardAmount(String(totalAmount.toFixed(2)));
                      setCashAmount("0");
                    } else {
                      // Remove $0.25 from card amount when unchecked
                      const newCardAmount = Math.max(0, currentCardAmount - 0.25);
          ...
          <div style={{
            ...
            {(Number(lottoWinnings) || 0) > 0 && (
              <div style={{ 
                ...
                💰 Customer won ${(Number(lottoWinnings) || 0).toFixed(2)} in lottery
              </div>
            )}
          </div>
                ...
                return (
                  <>
                    <Total>
                      ...
                      {discount > 0 ? (
                        <>
                          <div>Items Subtotal: ${originalSubtotal.toFixed(2)}</div>
                      ...
                      {creditCardCharge && (
                        <div style={{ color: "#6f42c1", fontWeight: "500" }}>
                      ...
                      {(Number(parseFloat(cashbackFee) || 0)) > 0 && (
                        <div style={{ color: "#d35400", fontWeight: "500" }}>
                          Cashback Fee: ${(Number(parseFloat(cashbackFee) || 0)).toFixed(2)}
                        </div>
                      )}
                      {lotto > 0 && (
                      ...
                      <div className="final-total">
                        {isBalancePayment ? (
                          <>
                            <div style={{ color: "#17a2b8", fontWeight: "600", marginBottom: "5px" }}>
                              🏦 Customer Balance Payment
                            </div>
                            <div>
                              Paying for: {selectedCustomer?.name || 'Selected Customer'}
                            </div>
                            <div>
                              Current Balance: ${selectedCustomer?.balance?.toFixed(2) || '0.00'}
                            </div>
                          </>
                        ) : (
                          <>Total Amount Due: ${finalTotal >= 0 ? finalTotal.toFixed(2) : `(${Math.abs(finalTotal).toFixed(2)}) Credit`}</>
                        )}
                      </div>
                    </Total>
                    ...
                    >
                        ...
                        {isCreditSale && (
                          ...
                          <div style={{ marginBottom: "0.75rem" }}>
                            <div style={{ 
                              ...
                              <label style={{ minWidth: "80px", fontSize: "0.9rem", color: "#856404", fontWeight: "500" }}>Customer:</label>
                              <div style={{ flex: 1, position: "relative", display: "flex", gap: "0.25rem" }}>
                                <input
                                  type="text"
                                ...
                                {showCustomerSuggestions && (
                                  <div style={{
                                    ...
                                    {(() => {
                                      const filteredCustomers = Array.isArray(existingCustomers) ? existingCustomers
                                        .filter(customer => 
                                          customer && customer.customerName && 
                                          customer.customerName.toLowerCase().includes(creditCustomerName.toLowerCase())
                                        ) : [];
                                      
                                      const exactMatch = filteredCustomers.find(customer => 
                                        customer.customerName.toLowerCase() === creditCustomerName.toLowerCase()
                                      );
                                          ...
                                          {filteredCustomers.map((customer, index) => (
                                            ...
                                            >
                                              ...
                                              <span style={{
                                                ...
                                                ${Math.abs(customer.balance || 0).toFixed(2)} {customer.balance > 0 ? "owed" : "credit"}
                                              </span>
                                            </div>
                                          ))}
                                          
                                          {/* Add New Customer option when no exact match found */}
                                          {creditCustomerName.trim().length > 0 && !exactMatch && (
                                            <div
                                              onClick={() => {
                                            ...
                                            >
                                              <span>➕</span>
                                              <span>Add "{creditCustomerName.trim()}" as new credit customer</span>
                                            </div>
                                          )}
                                        </>
                                      );
                            ...
                            {existingCustomers.length > 0 && (
                              <div style={{
                                ...
                                Loaded {existingCustomers.length} customers. {creditCustomerName && `Searching for: "${creditCustomerName}"`}
                              </div>
                            )}
                            ...
                            {selectedCustomer && (
                              <div style={{
                                ...
                                border: `1px solid ${customerBalance > 0 ? "#f5c6cb" : "#c3e6cb"}`,
                                borderRadius: "4px",
                                marginBottom: "0.5rem",
                                fontSize: "0.9rem"
                              }}>
                                ...
                                <span style={{
                                  ...
                                  ${Math.abs(customerBalance).toFixed(2)} {customerBalance > 0 ? "owed" : "credit"}
                                </span>
                                <div style={{ fontSize: "0.7rem", color: "#6c757d", marginTop: "0.25rem" }}>
                                  ID: {selectedCustomer.id} | Last updated: {new Date(selectedCustomer.updatedAt).toLocaleTimeString()}
                                </div>
                              </div>
                            )}
                        </div>                          {(() => {
                            ...
                            if (specifiedCreditAmount > 0) {
                              ...
                              return (
                                <div style={{ 
                                  ...
                                  border: `1px solid ${paymentStatus ? "#28a745" : "#ffc107"}`, 
                                  borderRadius: "4px", 
                                  padding: "0.5rem",
                                  fontSize: "0.9rem"
                                }}>
                                  <div><strong>Specified Credit Mode:</strong></div>
                                  <div>� Unpaid Balance: ${specifiedCreditAmount.toFixed(2)}</div>
                                  <div>💰 Payment Needed: ${paymentNeeded.toFixed(2)}</div>
                                  <div>📊 Payment Status: ${paidTotal.toFixed(2)} / ${paymentNeeded.toFixed(2)}</div>
                                  {!paymentStatus && (
                                    <div style={{ color: "#dc3545", fontWeight: "500", marginTop: "0.25rem" }}>
                                      ⚠️ Need ${(paymentNeeded - paidTotal).toFixed(2)} more
                                    </div>
                                  )}
                                </div>
                              );
                              ...
                              return (
                                <div style={{ 
                                  ...
                                  <div><strong>Auto-Calculate Mode:</strong></div>
                                  {paidCash > 0 && <div>💵 Cash: ${paidCash.toFixed(2)}</div>}
                                  {paidCard > 0 && <div>💳 Card: ${paidCard.toFixed(2)}</div>}
                                  <div style={{ color: "#dc3545", fontWeight: "500" }}>📝 Credit Balance: ${creditBalance.toFixed(2)}</div>
                                </div>
                              );
                              ...
                              return (
                                <div style={{ 
                                  ...
                                  ✅ <strong>Fully Paid - No Credit Balance</strong>
                                </div>
                              );
                            } else {
                              return (
                                <div style={{ 
                                  ...
                                  📝 <strong>Full Credit: ${creditBalance.toFixed(2)}</strong>
                                </div>
                              );
                            }
                          })()}
                      ...
                      >
                        ...
                        <button
                          ...
                          title={
                            isCreditSale ? "Terminal disabled in credit mode" : 
                            cart.length === 0 ? "Add items to cart first" : 
                      ...
                      {(Number(cashbackAmount) || 0) > 0 && (
                        <div style={{
                          ...
                          🚫 <strong>Cash payment disabled for cashback transactions</strong>
                          <div style={{ fontSize: "0.8rem", marginTop: "0.25rem", opacity: 0.9 }}>
                            Customer must pay by card • Cash register: -{(Number(cashbackAmount) || 0).toFixed(2)} + {(Number(cashbackFee) || 0).toFixed(2)} profit
                          </div>
                        </div>
                      )}
                          ...
                          {cashback > 0 && (
                            <div style={{ 
                              ...
                              <div style={{ fontSize: "0.85rem", color: "#856404" }}>
                                Requires card payment: ${(total + cashback).toFixed(2)} (Purchase: ${total.toFixed(2)} + Cashback: ${cashback.toFixed(2)})
                              </div>
                              <div style={{ fontSize: "0.8rem", color: "#d68910", marginTop: "0.25rem", fontWeight: "500" }}>
                                💳 Customer pays by card • 💵 You give ${cashback.toFixed(2)} cash from register
                              </div>
                            </div>
                          )}
                            ...
                            {isSufficient
                              ? `Card Payment: $${paidCard.toFixed(2)}${
                                  change > 0 && cashback === 0
                                    ...
                                    : ""
                                }${cashback > 0 ? ` — Cashback Given: $${cashback.toFixed(2)}` : ""}`
                              : cashback > 0 
                                ? `Card payment needed: $${(total + cashback).toFixed(2)} (Current: $${paidCard.toFixed(2)})`
                        ...
                        {(() => {
                          ...
                          if (isBalancePayment) {
                            ...
                            if (paymentTotal === 0) {
                              ...
                              return `Pay $${paymentTotal.toFixed(2)} (Balance Remaining: $${remainingBalance.toFixed(2)})`;
                            } else {
                              return `Pay $${paymentTotal.toFixed(2)} (Balance Paid)`;
                            }
                          }
                          ...
                          if (specifiedCreditAmount > 0) {
                            const paymentNeeded = finalTotal - specifiedCreditAmount;
                            return `Process Sale (Payment: $${paymentNeeded.toFixed(2)}, Credit: $${specifiedCreditAmount.toFixed(2)})`;
                          } else {
                            const creditBalance = Math.max(0, finalTotal - paidTotal);
                            if (paidTotal > 0 && creditBalance > 0) {
                              return `Process Partial Payment (Credit: $${creditBalance.toFixed(2)})`;
                            } else if (paidTotal >= finalTotal) {
...
export default function POSPage() {
  return (
    <Suspense fallback={<div style={{ padding: '2rem', textAlign: 'center' }}>Loading POS system...</div>}>
      <POSContent />
    </Suspense>
  );
}
