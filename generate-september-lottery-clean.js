...
async function generateSeptemberLotterySales() {
  ...
  try {
    ...
    console.log(
      "\n🎰 Generating September 2025 lottery sales targeting $25,800..."
    );

    // Check if September lottery data already exists
    const existingSeptemberLottery = await transactionCollection
      .find({
        timestamp: {
    ...
    if (existingSeptemberLottery.length > 0) {
      console.log(
        `⚠️  Found ${existingSeptemberLottery.length} existing September lottery transactions. Deleting first...`
      );
      ...
      console.log(
        `✅ Cleared existing September lottery data (${deleteResult.deletedCount} transactions)`
      );
    }
    ...
    let currentTransactionId = 92001; // Start from clean range for September lottery
    ...
    for (let day = 1; day <= 30; day++) {
      const date = new Date(
        `2025-09-${day.toString().padStart(2, "0")}T00:00:00.000Z`
      );
      const dayOfWeek = date.getDay(); // 0 = Sunday, 6 = Saturday

      // Lottery sales patterns by day of week - increased for higher target
    ...
    console.log(`\n✅ Verification:`);
    console.log(`   September lottery transactions in DB: ${verifyCount}`);
    console.log(
      `   Total September lottery sales: $${
    ...
    // Get other categories for comparison
    const groceryTotal = await transactionCollection
      .aggregate([
        {
          $match: {
            timestamp: {
    ...
    const alcoholTotal = await transactionCollection
      .aggregate([
        {
          $match: {
            timestamp: {
            ...
            items: { $elemMatch: { category: "Alcohol" } },
          },
        },
        {
          $group: { _id: null, total: { $sum: "$total" }, count: { $sum: 1 } },
    ...
    const tobaccoTotal = await transactionCollection
      .aggregate([
        {
          $match: {
            timestamp: {
    ...
    if (groceryTotal.length > 0) {
      console.log(
        `   Grocery: $${groceryTotal[0].total.toFixed(2)} (${
          groceryTotal[0].count
        } transactions)`
      );
    }
    if (alcoholTotal.length > 0) {
      console.log(
        `   Alcohol: $${alcoholTotal[0].total.toFixed(2)} (${
          alcoholTotal[0].count
        } transactions)`
      );
    }
    if (tobaccoTotal.length > 0) {
      console.log(
        `   Tobacco: $${tobaccoTotal[0].total.toFixed(2)} (${
          tobaccoTotal[0].count
        } transactions)`
      );
    }
    console.log(
