...
async function debugDatabaseConnection() {
  ...
  console.log(
    `   MONGO_DB: ${process.env.MONGO_DB || "convenience_store (default)"}`,
  );
  ...
  try {
    ...
    const novSearches = [
      ...
      {
        name: "Contains Nov 2025",
        query: { timestamp: { $regex: "2025-11" } },
      },
      {
        name: "Admin User cashier",
        query: { cashier: "Admin User", timestamp: { $regex: "2025-11" } },
      },
      {
        name: "TransactionId range",
        query: { transactionId: { $gte: 110800, $lt: 111800 } },
      },
    ];
