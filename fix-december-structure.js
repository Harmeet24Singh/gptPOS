...
async function fixDecemberDataStructure() {
    ...
    const lastTransaction = await db.collection("transactions").findOne(
        { transactionId: { $exists: true } },
        { sort: { transactionId: -1 } }
    );
    
    let nextTransactionId = lastTransaction ? lastTransaction.transactionId + 1 : 120000;
    ...
    console.log(`🔍 Found ${decemberTransactions.length} December transactions to fix`);
    ...
    for (let i = 0; i < decemberTransactions.length; i++) {
        ...
        const fixedItems = oldTx.items.map(item => ({
            barcode: item.barcode || `${Math.floor(Math.random() * 9000000000000) + 1000000000000}`,
            name: item.name,
            price: item.price,
        ...
        const fixedTransaction = {
            ...
            cashAmount: oldTx.paymentMethod === "cash" ? Math.round(total * 100) / 100 : 0,
            cardAmount: oldTx.paymentMethod === "card" ? Math.round(total * 100) / 100 : 0,
            creditAmount: 0,
            cashback: 0,
            change: 0
        };
    ...
    if (fixedTransactions.length > 0) {
        ...
        console.log(`✅ Inserted ${fixedTransactions.length} fixed December transactions`);
