...
async function thoroughOctoberCleanup() {
    ...
    try {
        ...
        console.log(`Grouped into ${Object.keys(transactionGroups).length} unique transactionIDs`);
        ...
        for (const [transId, transactions] of Object.entries(transactionGroups)) {
            if (transactions.length > 1) {
                console.log(`TransactionId ${transId} has ${transactions.length} entries`);
                ...
                for (const duplicate of toRemove) {
                    console.log(`    - ${duplicate.timestamp} ($${duplicate.total}) [${duplicate._id}]`);
        ...
        const newTotal = remainingTransactions.reduce((sum, t) => sum + parseFloat(t.total || 0), 0);
        const uniqueIds = new Set(remainingTransactions.map(t => t.transactionId)).size;
