...
async function enhanceNovemberAlcoholSales() {
  ...
  try {
    ...
    const MONTH_START = new Date("2025-11-01T00:00:00.000Z");
    const MONTH_END = new Date("2025-12-01T00:00:00.000Z");

    console.log(
      "\n🔧 Enhancing November 2025 alcohol sales with realistic patterns..."
    );

    // Step 1: Get all November alcohol transactions
    ...
    console.log(
      `📊 Found ${novemberTransactions.length} November alcohol transactions`
    );

    if (novemberTransactions.length === 0) {
      console.log(
        "❌ No November alcohol transactions found. Run generate-alcohol-sales-november.js first."
      );
      return;
    }
    ...
    // Weekly patterns (same as previous months)
    ...
    for (let day = 1; day <= 30; day++) {
      const date = new Date(2025, 10, day); // November
      ...
      transactionsByDay[day] = {
        ...
        expectedCount: Math.round(
          ((novemberTransactions.length * dayMultipliers[dayOfWeek]) / 30) * 7
        ),
        transactions: [],
      };
    }
    ...
    const shuffledTransactions = [...novemberTransactions];
    ...
    console.log("\n📅 November distribution after pattern application:");
    const weekSummary = {};
    ...
    Object.entries(weekSummary).forEach(([dayName, data]) => {
      ...
      const percentage = (
        (data.total / novemberTransactions.length) *
        100
      ).toFixed(1);
      console.log(
    ...
    console.log("\n📊 Final verification of November alcohol sales...");
    ...
    paymentDistribution.forEach((method) => {
      const percentage = (
        (method.count / novemberTransactions.length) *
        100
      ).toFixed(1);
      console.log(
        `${method._id}: ${
    ...
    console.log(`\n✅ November 2025 alcohol sales enhancement completed!`);
    console.log(
      `🎯 Target: $6,000 | Actual: $${finalStats[0]?.totalSales.toFixed(2)}`
    );
    console.log(
      `📊 ${novemberTransactions.length} transactions with realistic business patterns`
    );
  } catch (error) {
    console.error("Error enhancing November alcohol sales:", error);
  } finally {
    await client.close();
