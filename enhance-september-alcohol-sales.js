...
async function enhanceSeptemberAlcoholSales() {
  ...
  try {
    ...
    console.log(
      "\n🔧 Enhancing September 2025 alcohol sales with realistic patterns..."
    );

    // Step 1: Get all September alcohol transactions
    const septemberTransactions = await collection
      .find({
    ...
    console.log(
      `📊 Found ${septemberTransactions.length} September alcohol transactions`
    );

    if (septemberTransactions.length === 0) {
      console.log(
        "❌ No September alcohol transactions found. Run generate-alcohol-sales-september.js first."
      );
      return;
    }
    ...
    // Weekly patterns (same as July/August)
    ...
    for (let day = 1; day <= 30; day++) {
      const date = new Date(2025, 8, day); // September
      ...
      transactionsByDay[day] = {
        ...
        expectedCount: Math.round(
          ((septemberTransactions.length * dayMultipliers[dayOfWeek]) / 30) * 7
        ),
        transactions: [],
      };
    }
    ...
    const shuffledTransactions = [...septemberTransactions];
    ...
    console.log("\n📅 September distribution after pattern application:");
    const weekSummary = {};
    ...
    Object.entries(weekSummary).forEach(([dayName, data]) => {
      ...
      const percentage = (
        (data.total / septemberTransactions.length) *
        100
      ).toFixed(1);
      console.log(
    ...
    console.log("\n📊 Final verification of September alcohol sales...");
    ...
    paymentDistribution.forEach((method) => {
      const percentage = (
        (method.count / septemberTransactions.length) *
        100
      ).toFixed(1);
      console.log(
        `${method._id}: ${
    ...
    console.log(`\n✅ September 2025 alcohol sales enhancement completed!`);
    console.log(
      `🎯 Target: $8,000 | Actual: $${finalStats[0]?.totalSales.toFixed(2)}`
    );
    console.log(
      `📊 ${septemberTransactions.length} transactions with realistic business patterns`
    );
  } catch (error) {
    console.error("Error enhancing September alcohol sales:", error);
  } finally {
    await client.close();
