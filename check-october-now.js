const checkOctoberNow = async () => {
    try {
        ...
        if (totalSales > 75000) {
            console.log(`   ❌ STATUS: Still showing incorrect amount ($${totalSales.toFixed(2)})`);
            console.log(`   📝 The duplicates are still there. Database cleanup may not have worked.`);
        } else if (Math.abs(totalSales - 51330.76) < 5000) {
            console.log(`   ✅ STATUS: Much better! Close to target (difference: $${(totalSales - 51330.76).toFixed(2)})`);
        }
