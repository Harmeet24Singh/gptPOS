...
async function debugUnpaidCalculation() {
  ...
  try {
    ...
    julyTransactions.forEach((transaction, idx) => {
      ...
      if (transaction.paymentBreakdown && Array.isArray(transaction.paymentBreakdown)) {
        ...
        if (transaction.cashback > 0) {
          ...
          console.log(`   Cashback: $${transaction.cashback}, FinalTotal: $${transaction.finalTotal}`);
        } else {
        ...
        console.log(`   ⚠️  NO PAYMENT BREAKDOWN - Transaction counted as fully unpaid`);
    ...
    console.log(`📋 Problematic transactions: ${problematicTransactions.length}`);

    if (problematicTransactions.length > 0) {
      console.log(`\n⚠️  Problematic transactions:`);
      problematicTransactions.forEach(t => {
        console.log(`   ${t.id}: Total=$${t.total}, Paid=$${t.totalPaid || 'N/A'}, Unpaid=$${t.unpaid.toFixed(2)}`);
    ...
    console.log(`Transactions without payment breakdown: ${transactionsWithoutPaymentBreakdown}`);
