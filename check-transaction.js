...
async function checkTransaction() {
  ...
  try {
    ...
    console.log(
      `Range: ${beforeTime.toLocaleString()} to ${afterTime.toLocaleString()}`,
    );

    const timeRangeTransactions = await transactions
      .find({
        createdAt: {
          $gte: beforeTime,
          $lte: afterTime,
    ...
    console.log(
      `Found ${timeRangeTransactions.length} transactions in that time range:`,
    );
    timeRangeTransactions.forEach((t) => {
      console.log(
        `ID: ${t._id} | Total: $${t.total} | Type: ${t.transactionType || "N/A"} | Date: ${t.createdAt}`,
      );
    ...
    const todayTransactions = await transactions
      .find({
        createdAt: {
          $gte: today,
          $lt: tomorrow,
    ...
    const idMatches = await transactions
      .find({
    ...
    const negativeTransactions = await transactions
      .find({
        total: { $lt: 0 },
      })
      .sort({ createdAt: -1 })
    ...
    console.log(
      `Found ${negativeTransactions.length} recent negative transactions:`,
    );
