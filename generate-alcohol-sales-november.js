...
async function generateNovemberAlcoholSales() {
  ...
  try {
    ...
    const TARGET_AMOUNT = 6000;
    const MONTH_START = new Date("2025-11-01T00:00:00.000Z");
    const MONTH_END = new Date("2025-12-01T00:00:00.000Z");

    console.log(
      `\n🎯 Target: $${TARGET_AMOUNT} for November 2025 alcohol sales`
    );

    // Check if November data already exists
    ...
    if (existingNovember.length > 0) {
      console.log(
        `⚠️  Found ${existingNovember.length} existing November transactions. Deleting first...`
      );
      await collection.deleteMany({
        timestamp: { $gte: MONTH_START, $lt: MONTH_END },
      });
      console.log(`✅ Cleared existing November data`);
    }

    // Calculate number of transactions needed (even fewer for lower target)
    const avgTransactionAmount = TARGET_AMOUNT / 165; // Fewer transactions for lower target
    ...
    // Weekly business patterns (same as previous months)
    const dayPatterns = {
    ...
    // Beer and wine items (same variety as previous months)
    const alcoholItems = [
      {
    ...
    console.log(`\n📅 November 2025 distribution preview:`);
    const weekSummary = {};
    ...
    let currentAmount = 0;
    let transactionId = 110001; // November prefix: 11
    ...
    console.log(
      `✅ Successfully inserted ${insertResult.insertedCount} November 2025 alcohol transactions`
    );
    ...
    console.error("Error generating November alcohol sales:", error);
  } finally {
    await client.close();
    console.log("\nConnection closed");
  }
