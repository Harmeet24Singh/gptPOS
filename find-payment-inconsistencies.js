...
async function analyzeInconsistencies() {
    ...
    try {
        ...
        const decCard = await db.collection('transactions').findOne({ transactionId: 202364 });
        console.log(`   Transaction ID 202364 (card): cashAmount=${decCard.cashAmount}, cardAmount=${decCard.cardAmount}, total=${decCard.total}`);
        console.log(`   ❌ ERROR: Card transaction should have cardAmount = total, cashAmount = 0\n`);
        
        // Check overall pattern
        console.log("✅ CORRECT PATTERNS OBSERVED:");
        console.log("   1. October: cash transactions have cashAmount = total, cardAmount = 0");
        console.log("   2. November: cash transactions have cashAmount = total, cardAmount = 0");
        console.log("   3. December: cash transactions have cashAmount = total, cardAmount = 0");
        console.log("   4. January alcohol: card transactions have cardAmount = total, cashAmount = 0\n");
        
        // Count inconsistencies
        console.log("🔍 Finding all inconsistent transactions...\n");
        
        const inconsistentTransactions = await db.collection('transactions').find({
            $or: [
        ...
        console.log(`Found ${inconsistentTransactions.length} inconsistent transactions:`);
