...
async function investigateUnpaidAmount() {
  ...
  try {
    ...
    const creditUnpaid = await collection
      .find({
        timestamp: {
    ...
    console.log(
      `📋 Credit sales with unpaid status: ${creditUnpaid.length} transactions`
    );
    if (creditUnpaid.length > 0) {
      ...
      creditUnpaid.forEach((t, idx) => {
        ...
        if (idx < 10) {
          // Show first 10
          console.log(
            `  ${t.transactionId}: $${t.total} (creditStatus: ${t.creditStatus})`
          );
        }
      });
    ...
    const partialPayment = await collection
      .find({
        timestamp: {
          $gte: new Date("2025-07-01T00:00:00.000Z"),
          $lt: new Date("2025-08-01T00:00:00.000Z"),
    ...
    if (partialPayment.length > 0) {
      ...
      partialPayment.forEach((t, idx) => {
        ...
        if (idx < 10) {
          // Show first 10
          console.log(
            `  ${t.transactionId}: $${t.total} (balance: $${t.creditBalance})`
          );
        }
      });
      console.log(
        `  Total partial payment balance: $${totalPartialUnpaid.toFixed(2)}`
      );
    }

    // Check 3: Any transactions with credit-related fields
    const anyCredit = await collection
      .find({
        timestamp: {
          $gte: new Date("2025-07-01T00:00:00.000Z"),
          $lt: new Date("2025-08-01T00:00:00.000Z"),
    ...
    if (anyCredit.length > 0) {
      ...
      console.log(
        `\n💰 Calculated potential unpaid amount: $${potentialUnpaid.toFixed(
          2
        )}`
      );
    }
    ...
    console.log(
      `Total July sales: $${allJulyTransactions
    ...
    console.log(
      `Unpaid amount represents ${unpaidPercentage.toFixed(2)}% of total sales`
    );
  } catch (error) {
