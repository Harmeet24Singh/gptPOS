...
async function testMongoQueries() {
  ...
  const decemberWithAlcohol = await transactions
    .find({
      timestamp: { $gte: decemberStart, $lt: decemberEnd },
  ...
  console.log(
    `3. Sample December alcohol transaction timestamp: ${sampleAlcohol?.timestamp}`,
  );

  // Test 4: Count our generated alcohol sales specifically
  const generatedAlcohol = await transactions
    .find({
      timestamp: { $gte: decemberStart, $lt: decemberEnd },
      "items.category": "Alcohol",
  ...
  const recentDecember = await transactions
    .find({
      timestamp: { $gte: decemberStart, $lt: decemberEnd },
    })
    .sort({ timestamp: -1 })
  ...
  recentDecember.slice(0, 3).forEach((t, i) => {
    ...
    console.log(
      `   ${i + 1}. ${t.timestamp.toISOString()} - Alcohol: ${hasAlcohol}`,
    );
  });
