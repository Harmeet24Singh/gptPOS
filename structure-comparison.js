  ...
  try {
    ...
    for (const month of months) {
      const response = await fetch(
        `http://localhost:3000/api/transaction?dateFilter=month&monthFilter=${month.filter}&limit=1`,
        {
      ...
      if (data && data.length > 0) {
        ...
        console.log(
          `   timestamp: ${sample.timestamp} (type: ${typeof sample.timestamp})`,
        );
    ...
    const decAlcohol = await fetch(
      `http://localhost:3000/api/transaction?dateFilter=month&monthFilter=2025-12&limit=100`,
      {
        headers: { "x-api-key": "dev-secret" },
      },
    );
    ...
    const alcoholTrans = decData.filter((t) =>
      t.items?.some((item) => item.category === "Alcohol"),
    );

    console.log(
      `   December alcohol transactions found: ${alcoholTrans.length}`,
    );
    if (alcoholTrans.length > 0) {
      ...
      const alcoholItem = sample.items.find(
        (item) => item.category === "Alcohol",
      );
      console.log(
        `     Alcohol item: ${alcoholItem.name} - $${alcoholItem.price}`,
      );
    }
  } catch (error) {
