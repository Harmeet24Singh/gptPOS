...
async function main() {
  ...
  if (!fs.existsSync(backupDir)) {
    console.error('❌ No backups directory found. Run backup-mongodb.js first.');
    process.exit(1);
  }
  ...
  dates.forEach(date => {
    ...
    const backupsInDate = fs.readdirSync(datePath)
  ...
  if (isNaN(backupIndex) || backupIndex < 0 || backupIndex >= allBackups.length) {
  ...
  // Confirm restore
  const confirm = await askQuestion(`⚠️  This will REPLACE your current database with backup from:\n📅 ${selectedBackup.displayName}\n\nAre you sure? (type "CONFIRM" to proceed): `);
