...
async function forceCleanRestart() {
  ...
  try {
    ...
    const total = octoberData.reduce(
    ...
    if (total > 70000) {
      ...
      if (duplicateCount > 0) {
        ...
        const newTotal = cleanData.reduce(
          (sum, t) => sum + parseFloat(t.total || 0),
          0,
        );
        console.log(
          `   📊 After cleanup: ${cleanData.length} transactions, $${newTotal.toFixed(2)}`,
        );
      }
    } else {
