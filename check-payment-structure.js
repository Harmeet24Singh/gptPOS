  ...
  try {
    ...
    sampleTransactions.forEach((transaction, index) => {
      ...
      if (transaction.paymentBreakdown && Array.isArray(transaction.paymentBreakdown)) {
        console.log(`Payment Breakdown (${transaction.paymentBreakdown.length} entries):`);
        transaction.paymentBreakdown.forEach((payment, idx) => {
          console.log(`  ${idx}: method="${payment.method}", amount=${payment.amount}`);
        });
      } else {
      ...
      Object.keys(transaction).forEach(key => {
        if (key.toLowerCase().includes('payment') || key.toLowerCase().includes('cash') || key.toLowerCase().includes('card')) {
