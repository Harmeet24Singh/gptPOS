...
async function checkHighestTransactionId() {
    ...
    try {
        ...
        const highest = await db.collection('transactions').findOne(
            { transactionId: { $exists: true } },
            { sort: { transactionId: -1 } }
        );
        
        console.log(`Highest transaction ID: ${highest?.transactionId || 'None found'}`);
        
        // Also check if January 2026 data exists
        const januaryData = await db.collection('transactions').find({
            timestamp: {
