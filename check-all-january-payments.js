...
async function checkAllJanuaryPayments() {
  ...
  try {
    ...
    allJanuary.forEach((transaction) => {
      ...
      if (pm === undefined) {
        ...
        categoriesWithIssues[category] =
          (categoriesWithIssues[category] || 0) + 1;
      } else if (pm === null) {
    ...
    const problemTransactions = allJanuary
      .filter((t) => !t.paymentMethod || t.paymentMethod === "unknown")
      .slice(0, 5);
    ...
    if (problemTransactions.length > 0) {
      ...
      problemTransactions.forEach((t, i) => {
        console.log(
          `   ${i + 1}. ID ${t.transactionId}: ${t.paymentMethod || "UNDEFINED"} - Category: ${t.items?.[0]?.category || "None"} - Total: $${t.total}`,
        );
      });
    } else {
