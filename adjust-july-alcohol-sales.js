...
async function adjustJulyAlcoholSales() {
  ...
  try {
    ...
    const julyAlcoholTransactions = await collection
      ...
      .sort({ total: -1 })
      .toArray(); // Sort by highest value first

    const currentTotal = julyAlcoholTransactions.reduce(
      (sum, t) => sum + t.total,
      0
    );
    const target = 13650;
    ...
    console.log(
      `📈 New total will be: $${(currentTotal - totalToRemove).toFixed(2)}`
    );
    ...
    console.log(
      `📈 Difference from target: $${(newTotal - target).toFixed(2)}`
    );
