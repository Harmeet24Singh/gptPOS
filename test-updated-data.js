const testUpdatedData = async () => {
  try {
    ...
    const response = await fetch(
      `http://localhost:3000/api/transaction?dateFilter=month&monthFilter=2025-12&limit=10&sortBy=timestamp&sortOrder=desc`,
      {
      ...
      data.forEach((transaction, index) => {
        ...
        if (transaction.items && transaction.items.length > 0) {
          transaction.items.forEach((item) => {
            console.log(
              `   Item: ${item.name} (${item.category}) - $${item.price} [${item.barcode || "NO BARCODE"}]`,
            );
          });
        }
      });
    ...
    const alcoholResponse = await fetch(
      `http://localhost:3000/api/transaction?dateFilter=month&monthFilter=2025-12&category=Alcohol&limit=5`,
      {
        headers: { "x-api-key": "dev-secret" },
      },
    );
    ...
    if (Array.isArray(alcoholData)) {
      console.log(
        `Found ${alcoholData.length} alcohol transactions in December`,
      );

      alcoholData.forEach((transaction, index) => {
        const alcoholItems = transaction.items.filter(
        ...
        alcoholItems.forEach((item) => {
          console.log(
            `   ${item.name} - $${item.price} [${item.barcode || "NO BARCODE"}]`,
          );
        });
      });
    }
