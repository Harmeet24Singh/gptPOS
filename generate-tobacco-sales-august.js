...
async function generateTobaccoSalesAugust() {
  ...
  try {
    ...
    console.log(
      "\n🚬 Generating August 2025 tobacco sales targeting $10,190..."
    );
    ...
    // Get starting transaction ID for August
    ...
    let currentTransactionId = 81001; // Start from clean range for August tobacco
    ...
    // Target and tracking
    const targetAmount = 10190;
    let totalGenerated = 0;
    const transactions = [];

    // Payment method distribution (same as July tobacco: 63.4% cash, 36.6% card)
    const paymentMethods = [
      { method: "cash", percentage: 63 },
      { method: "card", percentage: 37 },
    ];

    // Business hours: 7 AM to 9 PM
    const businessHours = { start: 7, end: 21 };

    // Generate transactions throughout August 2025
    const daysInAugust = 31;

    console.log(`📅 Generating tobacco transactions for August 1-31, 2025...`);
    ...
    while (totalGenerated < targetAmount) {
      ...
      for (let i = 0; i < itemCount; i++) {
        ...
        // Quantity logic: same as July tobacco sales
        let quantity;
      ...
      if (transactions.length > 800) {
        console.log("⚠️  Reached 800 transactions, stopping generation");
        break;
      }
    }
    ...
    if (transactions.length > 0) {
      ...
      console.log(`\n✅ Verification:`);
      console.log(`   August tobacco transactions in DB: ${verifyCount}`);
      console.log(
        `   Total August tobacco sales: $${
          verifyTotal[0]?.total.toFixed(2) || 0
        }`
      );

      // Final summary with all categories for August
      ...
      console.log(`\n🎉 August 2025 tobacco sales generation complete!`);
    } else {
      console.log("❌ No transactions generated");
    }
  } catch (error) {
    console.error("Error generating August tobacco sales:", error);
  } finally {
    await client.close();
  }
}

generateTobaccoSalesAugust();
