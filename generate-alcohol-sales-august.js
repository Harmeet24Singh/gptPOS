...
async function generateAugustAlcoholSales() {
  ...
  try {
    ...
    const TARGET_AMOUNT = 11550;
    const MONTH_START = new Date("2025-08-01T00:00:00.000Z");
    const MONTH_END = new Date("2025-09-01T00:00:00.000Z");

    console.log(`\n🎯 Target: $${TARGET_AMOUNT} for August 2025 alcohol sales`);

    // Check if August data already exists
    ...
    if (existingAugust.length > 0) {
      console.log(
        `⚠️  Found ${existingAugust.length} existing August transactions. Deleting first...`
      );
      await collection.deleteMany({
        timestamp: { $gte: MONTH_START, $lt: MONTH_END },
      });
      console.log(`✅ Cleared existing August data`);
    }

    // Calculate number of transactions needed (based on July's average)
    const avgTransactionAmount = TARGET_AMOUNT / 325; // Slightly fewer transactions than July (382)
    ...
    // Weekly business patterns (same as July)
    const dayPatterns = {
    ...
    // Beer and wine items (same variety as July)
    const alcoholItems = [
      {
    ...
    console.log(`\n📅 August 2025 distribution preview:`);
    const weekSummary = {};
    ...
    let currentAmount = 0;
    let transactionId = 80001; // August prefix: 8
    ...
    console.log(
      `✅ Successfully inserted ${insertResult.insertedCount} August 2025 alcohol transactions`
    );
    ...
    console.error("Error generating August alcohol sales:", error);
  } finally {
    await client.close();
    console.log("\nConnection closed");
  }
