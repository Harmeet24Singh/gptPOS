...
async function comprehensiveDecemberUpdate() {
    ...
    try {
        ...
        const incompleteTransactions = await collection.find({
            $and: [
        ...
        console.log(`Found ${incompleteTransactions.length} December transactions needing updates`);
        ...
        let nextTransactionId = lastTransaction ? lastTransaction.transactionId + 1 : 202400;
        ...
        for (let i = 0; i < incompleteTransactions.length; i += batchSize) {
            ...
            for (const transaction of batch) {
                ...
                if (transaction.items && transaction.items.some(item => !item.barcode)) {
                ...
                if (updated % 50 === 0) {
                    console.log(`Updated ${updated}/${incompleteTransactions.length} transactions...`);
                }
            }
        }
