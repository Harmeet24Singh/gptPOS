...
async function checkTimestampFormats() {
    ...
    samples.forEach((t, i) => {
        ...
        console.log(`   ISO String: ${t.timestamp instanceof Date ? t.timestamp.toISOString() : 'Not a Date'}`);
        console.log(`   Cashier: ${t.cashier}`);
        console.log(`   Items: ${t.items.filter(item => item.category === "Alcohol").map(item => item.name)}`);
        console.log("");
    });
    ...
    const testQueries = [
        ...
        { timestamp: { $gte: new Date("2025-12-01T00:00:00.000Z"), $lt: new Date("2026-01-01T00:00:00.000Z") } },
        // Test as strings
    ...
    for (let i = 0; i < testQueries.length; i++) {
        const count = await db.collection("transactions").countDocuments(testQueries[i]);
        console.log(`Query ${i+1} (${Object.keys(testQueries[i])[0]}): ${count} results`);
    }
