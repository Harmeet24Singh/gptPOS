...
async function compareMonthlyDataStructure() {
    ...
    const months = [
        { name: "October", query: { timestamp: { $gte: new Date("2025-10-01"), $lt: new Date("2025-11-01") } } },
        { name: "November", query: { timestamp: { $gte: new Date("2025-11-01"), $lt: new Date("2025-12-01") } } },
        { name: "December", query: { $or: [
            { timestamp: { $gte: "2025-12-01T00:00:00.000Z", $lt: "2026-01-01T00:00:00.000Z" } },
            { timestamp: { $gte: new Date("2025-12-01"), $lt: new Date("2026-01-01") } }
        ] } }
    ];
    
    for (const month of months) {
        const transactions = await db.collection("transactions").find(month.query).toArray();
        
        if (transactions.length > 0) {
            const totalSales = transactions.reduce((sum, t) => sum + (t.total || 0), 0);
            const avgTransaction = totalSales / transactions.length;
            
            const cashCount = transactions.filter(t => t.transactionType === "cash").length;
            const cardCount = transactions.filter(t => t.transactionType === "card").length;
            
            console.log(`\n   ${month.name}:`);
            console.log(`     Transactions: ${transactions.length}`);
            console.log(`     Total Sales: $${totalSales.toFixed(2)}`);
            console.log(`     Avg Transaction: $${avgTransaction.toFixed(2)}`);
            console.log(`     Cash: ${cashCount} (${(cashCount/transactions.length*100).toFixed(1)}%)`);
            console.log(`     Card: ${cardCount} (${(cardCount/transactions.length*100).toFixed(1)}%)`);
