...
const debugMonthFilter = async () => {
  try {
    const response = await fetch(
      "http://localhost:3000/api/transaction?dateFilter=month&monthFilter=2025-12",
      {
      ...
      data.forEach((transaction, index) => {
        if (transaction.items && Array.isArray(transaction.items)) {
          ...
          if (alcoholItems.length > 0) {
            hasAlcoholItems++;
            if (hasAlcoholItems <= 3) {
              // Show first 3
              console.log(
                `\nTransaction ${index} has alcohol:`,
                alcoholItems.map((item) => item.name),
              );
            }
          }
        }
