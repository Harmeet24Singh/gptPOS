  ...
  try {
    ...
    for (let i = 0; i < julyTransactions.length; i++) {
      ...
      if (updateResult.modifiedCount > 0) {
        updateCount++;
        if (updateCount % 50 === 0) {
          console.log(`Shuffled ${updateCount}/${julyTransactions.length} transactions...`);
        }
      }
    }
    ...
    const firstShuffled = await collection.find({
      timestamp: {
        $gte: new Date("2025-07-01T00:00:00.000Z"),
        $lt: new Date("2025-08-01T00:00:00.000Z"),
      }
    }).sort({ transactionId: 1 }).limit(20).toArray();

    firstShuffled.forEach((t, idx) => {
      console.log(`${(idx + 1).toString().padStart(2)}: ${t.transactionId} - ${t.paymentMethod.padEnd(6)} - $${t.total.toFixed(2)}`);
    });

    // Show new distribution in first 50
    const newFirst50 = await collection.find({
      timestamp: {
        $gte: new Date("2025-07-01T00:00:00.000Z"),
        $lt: new Date("2025-08-01T00:00:00.000Z"),
      }
