const checkSpecificAlcohol = async () => {
  try {
    ...
    const response = await fetch(
      `http://localhost:3000/api/transaction?dateFilter=month&monthFilter=2025-12&limit=2000`,
      {
    ...
    const alcoholTransactions = allData.filter(
      (transaction) =>
        transaction.items &&
    ...
    console.log(
      `🍺 Alcohol transactions found: ${alcoholTransactions.length}\n`,
    );
    ...
    if (alcoholTransactions.length > 0) {
      ...
      alcoholTransactions.slice(0, 3).forEach((transaction, index) => {
        ...
        const otherItems = transaction.items.filter(
          (item) => item.category !== "Alcohol",
        );
