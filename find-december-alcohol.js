...
async function findDecemberAlcohol() {
    ...
    const alcoholTransactions = await db.collection("transactions").find({
    ...
    alcoholTransactions.forEach((t, i) => {
        ...
        console.log(`   ISO: ${t.timestamp instanceof Date ? t.timestamp.toISOString() : t.timestamp}`);
        console.log(`   Items: ${t.items.map(item => `${item.name} (${item.category})`).join(', ')}`);
        console.log("");
    });
    ...
    alcoholTransactions.forEach((t, i) => {
        ...
        console.log(`${i+1}. ${timestamp.toISOString()} - In December? ${inRange}`);
    });
