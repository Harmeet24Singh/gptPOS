...
async function debugMarch2025Issues() {
  ...
  try {
    ...
    marchSamples.forEach((t, i) => {
      ...
      console.log(
        `      paymentMethod: "${t.paymentMethod}" (type: ${typeof t.paymentMethod})`,
      );
      console.log(
        `      cashAmount: ${t.cashAmount} (type: ${typeof t.cashAmount})`,
      );
      console.log(
        `      cardAmount: ${t.cardAmount} (type: ${typeof t.cardAmount})`,
      );
      console.log(`      cashier: "${t.cashier}"`);
      console.log(`      store: "${t.store}"`);
      console.log(
        `      paymentBreakdown: ${JSON.stringify(t.paymentBreakdown)}`,
      );
      console.log("");
    });
    ...
    categoryTotals.forEach((cat) => {
      console.log(`   ${cat._id}:`);
      console.log(
        `      Total Sales: $${cat.totalSales} (type: ${typeof cat.totalSales})`,
      );
    ...
    if (invalidTransactions.length > 0) {
      invalidTransactions.slice(0, 3).forEach((t) => {
        console.log(
          `      ID ${t.transactionId}: paymentMethod="${t.paymentMethod}", total=${t.total}, items=${t.items.length}`,
        );
      });
    }
    ...
    tobaccoLottery.forEach((t) => {
      ...
      console.log(
        `      Items: ${t.items.map((i) => `${i.name} ($${i.price})`).join(", ")}`,
      );
    });
    ...
    if (julySample) {
      ...
      console.log(
        `      paymentBreakdown: ${JSON.stringify(julySample.paymentBreakdown)}`,
      );
    }
    ...
    if (marchSample) {
      ...
      console.log(
        `      paymentBreakdown: ${JSON.stringify(marchSample.paymentBreakdown)}`,
      );
    }
  } catch (error) {
