...
async function testApril2025() {
  ...
  try {
    ...
    for (const [category, target] of Object.entries(categoryTargets)) {
      ...
      while (
        categoryGenerated < remainingTarget * 0.98 &&
        categoryCount < 2000
      ) {
        const avgTransactionAmount = Math.max(
          5,
          remainingTarget / Math.max(50, Math.floor(remainingTarget / 15)),
        );
        const transactionAmount =
          avgTransactionAmount * (0.7 + Math.random() * 0.6);
