...
async function testApril2025() {
    console.log("🧪 TEST RUN: APRIL 2025 SALES GENERATION");
    ...
    const totalTarget = Object.values(categoryTargets).reduce((sum, val) => sum + val, 0);
    const month = 4; // April
    const year = 2025;
    
    console.log(`\n📅 TEST: Generating sales for April 2025`);
    console.log(`🎯 Targets:`);
    ...
    try {
        ...
        // Check existing April 2025 sales by category (CORRECT METHOD)
        const existingSalesByCategory = await db.collection('transactions').aggregate([
            ...
            },
            {
                $group: {
                    _id: {
                        $cond: {
                            if: { $in: ["Alcohol", "$items.category"] },
                            then: "Alcohol",
                            else: {
                                $cond: {
                                    if: { $in: ["Grocery", "$items.category"] },
                                    then: "Grocery", 
                                    else: {
                                        $cond: {
                                            if: { $in: ["Tobacco", "$items.category"] },
                                            then: "Tobacco",
                                            else: {
                                                $cond: {
                                                    if: { $in: ["Lottery", "$items.category"] },
                                                    then: "Lottery",
                                                    else: "Other"
                                                }
                                            }
        ...
        console.log(`\n📊 Current April 2025 sales by category:`);
        ...
        for (const [category, target] of Object.entries(categoryTargets)) {
            ...
            // IMPROVED ALGORITHM - ensure we actually reach the target
        ...
        console.log(`\n🎉 TEST SUCCESSFUL!`);
        console.log(`📊 April 2025 Generated Sales Summary:`);
        ...
        console.log(`   🎯 Card Limit: ${finalTotalCard <= MAX_CARD_SALES ? '✅ Respected' : '❌ Exceeded'} ($${MAX_CARD_SALES} max)`);
        
        // Final verification with CORRECT method
        console.log(`\n🔍 Final April 2025 Totals by Category (Frontend Method):`);
        const finalTotals = await db.collection('transactions').aggregate([
            {
                $match: {
                    timestamp: { 
                        $gte: new Date(2025, 3, 1),
                        $lt: new Date(2025, 4, 1)
                    }
                }
            },
            {
        ...
        finalTotals.forEach(cat => {
            ...
            const achievement = target > 0 ? (cat.total / target * 100).toFixed(1) : '0';
            console.log(`   ${cat._id}: $${cat.total.toFixed(2)} / $${target} (${achievement}% achieved)`);
        });
