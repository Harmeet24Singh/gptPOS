...
async function compareJulyVsMarch() {
    ...
    try {
        ...
        if (julyTransaction && marchTransaction) {
            ...
            const missingInMarch = julyFields.filter(field => !marchFields.includes(field));
            const extraInMarch = marchFields.filter(field => !julyFields.includes(field));
            ...
            julyFields.forEach(field => {
                if (marchFields.includes(field)) {
                    ...
                    if (typeof julyValue !== typeof marchValue) {
                        console.log(`   ${field}: July=${typeof julyValue}, March=${typeof marchValue}`);
                    }
                }
            });
        }
