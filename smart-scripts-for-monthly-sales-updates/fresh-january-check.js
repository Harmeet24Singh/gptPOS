...
async function getCurrentTotals() {
  ...
  try {
    ...
    results.forEach((cat) => {
      ...
      console.log(
        `${emoji} ${cat._id}: $${cat.totalAmount.toFixed(2)} (${cat.transactionCount} transactions) ${status}`,
      );
    });
    ...
    console.log(
      `💳 Card Sales: $${totalCard.toFixed(2)} ${totalCard > 15000 ? "⚠️ OVER LIMIT" : "✅ WITHIN LIMIT"}`,
    );
