...
async function checkJanuaryTotals() {
    ...
    try {
        ...
        // Get overall totals
        const overall = await db.collection('transactions').aggregate([
            {
                $match: {
                    timestamp: { 
                        $gte: new Date(2026, 0, 1),
        ...
        categories.forEach(cat => {
            ...
            console.log(`${getCategoryEmoji(cat._id)} ${cat._id}: $${cat.totalAmount.toFixed(2)} (${cat.transactionCount} transactions) ${status}`);
        });
        ...
        console.log(`💳 Card Sales: $${totals.cardSales.toFixed(2)} ${totals.cardSales > 15000 ? '⚠️ OVER LIMIT' : '✅'}`);
