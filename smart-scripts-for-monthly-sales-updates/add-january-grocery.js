...
async function addGroceryTransactions() {
  ...
  try {
    ...
    while (generatedAmount < needed * 0.98 && transactionCount < 500) {
      // 98% of target, max 500 transactions
    ...
    console.log(
      `🎯 Will achieve: $${(currentTotal + generatedAmount).toFixed(2)}`,
    );
    ...
    if (transactions.length > 0) {
      ...
      console.log(
        `🎯 Target achievement: ${((finalTotal / target) * 100).toFixed(1)}%`,
      );
    }
  } catch (error) {
