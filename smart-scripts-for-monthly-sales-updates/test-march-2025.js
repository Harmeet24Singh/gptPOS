...
function generateTransaction(transactionId, month, year, category, targetAmount, forcePaymentMethod = null) {
    ...
    const subtotal = parseFloat(items.reduce((sum, item) => sum + item.price, 0).toFixed(2));
    const totalTax = parseFloat(items.reduce((sum, item) => sum + item.tax, 0).toFixed(2));
...
async function testMarch2025() {
    ...
    try {
        ...
        for (const [category, target] of Object.entries(categoryTargets)) {
            ...
            while (categoryGenerated < remainingTarget * 0.95 && categoryCount < 500) {
                const avgTransactionAmount = remainingTarget / Math.max(10, Math.floor(remainingTarget / 25));
