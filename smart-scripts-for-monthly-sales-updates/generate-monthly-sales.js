...
async function generateMonthlySales() {
  ...
  const totalTarget =
    alcoholTarget + groceryTarget + tobaccoTarget + lotteryTarget;
  ...
  try {
    ...
    const existingCardSales = await db
      .collection("transactions")
      .aggregate([
        {
          $match: {
    ...
    if (totalCardAmount >= MAX_CARD_SALES) {
      console.log(
        `⚠️  Card limit already reached - all new transactions will be cash only`,
      );
    }
    ...
    console.log(
      `   🎯 Final Card Total: $${totalCardAmount.toFixed(2)} (${totalCardAmount <= MAX_CARD_SALES ? "✅ Within" : "❌ Exceeds"} $${MAX_CARD_SALES} limit)`,
    );
    console.log(`   💳 Card Generated: $${finalTotalCard.toFixed(2)}`);
    console.log(
      `   📊 Month Status: ${totalCardAmount <= MAX_CARD_SALES ? "Compliant" : "Over Limit"}`,
    );
  } catch (error) {
