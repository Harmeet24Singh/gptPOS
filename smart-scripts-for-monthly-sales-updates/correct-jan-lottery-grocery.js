...
async function correctJanuaryLotteryAndGrocery() {
    ...
    try {
        ...
        console.log('\\n📊 CURRENT STATUS:');
        console.log(`🎲 Lottery: $${lotteryTotal.toFixed(2)} (${lotteryCount} transactions)`);
        console.log(`🛒 Grocery: $${groceryTotal.toFixed(2)} (${groceryCount} transactions)`);
        console.log('\\n🎯 TARGETS:');
        ...
        if (lotteryTotal > lotteryTarget) {
            ...
            console.log(`   🗑️  Deleting ${lotteryToDelete.length} lottery transactions ($${lotteryDeletedAmount.toFixed(2)})`);
            
            const lotteryDeleteResult = await db.collection('transactions').deleteMany({
                _id: { $in: lotteryToDelete }
            });
            
            console.log(`   ✅ Deleted ${lotteryDeleteResult.deletedCount} lottery transactions`);
        } else {
        ...
        if (Math.abs(groceryDifference) > 200) { // Only correct if difference is significant
            if (groceryDifference > 0) {
                // Too much grocery - remove some
                console.log(`\\n🛒 CORRECTING GROCERY - Need to remove $${groceryDifference.toFixed(2)}`);
                
                const groceryTransactions = await db.collection('transactions').find({
                    timestamp: { 
                        $gte: new Date(2026, 0, 1),
                        $lt: new Date(2026, 1, 1)
                    },
                ...
                console.log(`   🗑️  Deleting ${groceryToDelete.length} grocery transactions ($${groceryDeletedAmount.toFixed(2)})`);
                
                const groceryDeleteResult = await db.collection('transactions').deleteMany({
                    _id: { $in: groceryToDelete }
                });
                
                console.log(`   ✅ Deleted ${groceryDeleteResult.deletedCount} grocery transactions`);
                
            } else {
                // Not enough grocery - add some
                console.log(`\\n🛒 GROCERY NEEDS INCREASE - Need to add $${Math.abs(groceryDifference).toFixed(2)}`);
                console.log('   ℹ️  Use the generate-monthly-sales.js script to add grocery transactions');
            }
        } else {
        ...
        console.log('==========================');
        console.log(`🎲 Final lottery: $${finalLotteryTotal.toFixed(2)} (${finalLotteryCount} transactions)`);
        console.log(`   Status: ${finalLotteryTotal < lotteryTarget ? '✅ Under target' : '❌ Still over target'}`);
        console.log(`🛒 Final grocery: $${finalGroceryTotal.toFixed(2)} (${finalGroceryCount} transactions)`);
        console.log(`   Status: ${Math.abs(finalGroceryTotal - groceryTarget) < 500 ? '✅ Near target' : '⚠️  Needs adjustment'}`);
