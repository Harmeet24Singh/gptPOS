...
async function aggressiveLotteryCorrection() {
  ...
  try {
    ...
    for (const transaction of lotteryTransactions) {
      ...
      console.log(
        `➖ Will remove: $${transaction.total.toFixed(2)} (ID: ${transaction._id})`,
      );
    }
    ...
    console.log(
      `🎯 New projected total: $${(currentTotal - removedAmount).toFixed(2)}`,
    );
    ...
    if (transactionsToRemove.length > 0) {
      ...
      const remainingLottery = await db
        .collection("transactions")
        .find({
          timestamp: {
            $gte: new Date(2026, 0, 1),
      ...
      console.log(
        `🎯 Status: ${finalTotal < 19000 ? "✅ Under $19,000 limit" : "⚠️ Still over limit"}`,
      );
    }
  } catch (error) {
