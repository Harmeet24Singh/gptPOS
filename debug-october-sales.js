  ...
  try {
    ...
    data.forEach((transaction) => {
      ...
      if (duplicateCheck.has(key)) {
        duplicates++;
        console.log(
          `🔴 DUPLICATE: TransactionId ${transId}, Timestamp: ${timestamp}, Total: $${total}`,
        );
      } else {
    ...
    console.log(
      `❗ Difference: $${(totalSales - 51330.76).toFixed(2)} too much`,
    );
      ...
      if (transactions.length > 1) {
        ...
        if (duplicateTransIds <= 5) {
          ...
          transactions.forEach((t, index) => {
            console.log(
              `   ${index + 1}. ${t.timestamp} - $${t.total} (MongoDB ID: ${t.id})`,
            );
          });
        }
      }
    ...
    console.log(`\n📊 Summary:`);
    console.log(
      `   Total Unique TransactionIDs: ${Object.keys(transIdGroups).length}`,
    );
    ...
    console.log(
      `   Date Range: ${timestamps[0].toISOString().split("T")[0]} to ${timestamps[timestamps.length - 1].toISOString().split("T")[0]}`,
    );
  } catch (error) {
