...
async function checkNovemberData() {
  ...
  if (allNovember > 0) {
    ...
    samples.forEach((t, i) => {
      console.log(
        `${i + 1}. ${t.timestamp} - ${t.items?.map((item) => item.name).join(", ")}`,
      );
      console.log(
        `   Categories: ${t.items?.map((item) => item.category).join(", ")}`,
      );
    ...
    categoryTotals.forEach((cat) => {
      console.log(
        `   ${cat._id}: $${cat.totalSales.toFixed(2)} (${cat.transactionCount} items)`,
      );
    });
  }
