...
async function updateJulyPaymentStructure() {
  ...
  try {
    ...
    const cashCount = Math.floor(totalTransactions * 0.70); // 70% cash
    ...
    console.log(`Cash only: ${cashCount} transactions (${(cashCount/totalTransactions*100).toFixed(1)}%)`);
    console.log(`Card only: ${cardCount} transactions (${(cardCount/totalTransactions*100).toFixed(1)}%)`);
    console.log(`Mixed: ${mixedCount} transactions (${(mixedCount/totalTransactions*100).toFixed(1)}%)`);
    ...
    for (let i = 0; i < julyTransactions.length; i++) {
      ...
      if (i < cashCount) {
        ...
        paymentBreakdown = [
          { method: cardType, amount: transaction.total }
        ];
      } else {
      ...
      if (updateResult.modifiedCount > 0) {
        updateCount++;
        if (updateCount % 50 === 0) {
          console.log(`Updated ${updateCount}/${julyTransactions.length} transactions...`);
        }
      }
    }
