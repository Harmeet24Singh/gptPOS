...
async function addSuperAdminToAtlas() {
  ...
  console.log('MONGO_URI:', process.env.MONGO_URI || 'mongodb://localhost:27017');
  ...
  try {
    ...
    if (savedUser) {
      console.log('✅ User verification successful - user exists in Atlas database');
      console.log('User ID:', savedUser.id);
      console.log('User Role:', savedUser.role);
    } else {
      console.log('❌ User verification failed - user not found in Atlas database');
    }
