...
async function fixAllPaymentInconsistencies() {
  ...
  try {
    ...
    console.log(
      `   ✅ Updated ${cardResult.modifiedCount} card transactions\n`,
    );
    ...
    console.log(
      `   ✅ Updated ${cashResult.modifiedCount} cash transactions\n`,
    );
    ...
    const creditResult = await db
      .collection("transactions")
      .updateMany({ paymentMethod: "credit" }, [
        {
          $set: {
            creditAmount: "$total",
            cashAmount: 0,
    ...
    console.log(
      `   ✅ Updated ${creditResult.modifiedCount} credit transactions\n`,
    );

    // Verify the fix
    console.log("🔍 Verifying fix...");
    const stillInconsistent = await db
      .collection("transactions")
      .find({
        $or: [
