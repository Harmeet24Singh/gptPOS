...
async function generateTobaccoSalesNovember() {
  ...
  try {
    ...
    console.log(
      "\n🚬 Generating November 2025 tobacco sales targeting $5,200..."
    );
    ...
    // Get starting transaction ID for November
    ...
    let currentTransactionId = 111001; // Start from clean range for November tobacco
    ...
    // Generate transactions throughout November 2025
    const daysInNovember = 30;

    console.log(
      `📅 Generating tobacco transactions for November 1-30, 2025...`
    );

    while (totalGenerated < targetAmount) {
      // Random day in November
      const day = Math.floor(Math.random() * daysInNovember) + 1;
      const hour =
        Math.floor(Math.random() * (businessHours.end - businessHours.start)) +
        businessHours.start;
      const minute = Math.floor(Math.random() * 60);

      const timestamp = new Date(2025, 10, day, hour, minute); // Month is 0-indexed (10 = November)
      ...
      if (totalGenerated + total > targetAmount + 150) {
        continue;
      }
      ...
      if (transactions.length > 400) {
        console.log("⚠️  Reached 400 transactions, stopping generation");
        break;
      }
    }
    ...
    if (transactions.length > 0) {
      ...
      console.log(`\n✅ Verification:`);
      console.log(`   November tobacco transactions in DB: ${verifyCount}`);
      console.log(
        `   Total November tobacco sales: $${
          verifyTotal[0]?.total.toFixed(2) || 0
        }`
      );

      // Final summary with all categories for November
      console.log(`\n🏁 November 2025 Sales Summary:`);

      // Get all November sales by category
      ...
      console.log(`\n🎉 November 2025 tobacco sales generation complete!`);
    } else {
      console.log("❌ No transactions generated");
    }
  } catch (error) {
    console.error("Error generating November tobacco sales:", error);
  } finally {
    await client.close();
  }
}

generateTobaccoSalesNovember();
