...
async function saveTransaction(txObj) {
  ...
  const doc = {
    ...
    items: Array.isArray(txObj.items)
      ? txObj.items.map((it) => ({
  ...
  return accounts.map((account) => ({
    ...
    updatedAt:
      account.updatedAt instanceof Date
        ...
        : account.updatedAt,
    lastTransactionDate:
      account.lastTransactionDate instanceof Date
        ? account.lastTransactionDate.toISOString()
...
async function upsertCreditAccount(accountData) {
  ...
  if (existingAccount) {
    ...
    console.log(
      `Updating account ${customerName}: Old balance=${existingAccount.balance}, New balance=${updateData.balance}`
    );

    // Update last transaction date if provided
    if (accountData.lastTransactionDate) {
      updateData.lastTransactionDate =
        accountData.lastTransactionDate instanceof Date
          ? accountData.lastTransactionDate
          : new Date(accountData.lastTransactionDate);
    }
    ...
    const result = await db
      .collection("creditAccounts")
      .updateOne({ _id: existingAccount._id }, { $set: updateData });

    if (result.modifiedCount > 0) {
      const updatedAccount = await db
        .collection("creditAccounts")
        .findOne({ _id: existingAccount._id });
      return {
        ...updatedAccount,
...
async function payToCreditAccount(customerName, paymentAmount) {
  ...
  console.log(
    `Processing payment for ${customerName}: Balance=${currentBalance}, Payment=${payment}`
  );
...
async function searchCreditAccounts(searchTerm, includeInactive = false) {
  ...
  const cursor = db
    .collection("creditAccounts")
    .find(query)
    .sort({ balance: -1, lastTransactionDate: -1 });
  const accounts = await cursor.toArray();
