  ...
  for (const month of months) {
    try {
      const response = await fetch(
        `http://localhost:3000/api/transaction?dateFilter=month&monthFilter=${month.filter}&limit=10`,
        {
      ...
      if (Array.isArray(data)) {
        const alcoholCount = data.filter((t) =>
          t.items?.some((item) => item.category === "Alcohol"),
        ).length;
        const groceryCount = data.filter((t) =>
