...
async function verifyDecemberAlcohol() {
  ...
  try {
    ...
    const decemberAlcohol = await collection.find({
    ...
    if (decemberAlcohol.length > 0) {
      ...
      sample.items.forEach(item => {
        console.log(`  - ${item.name} (${item.category}) $${item.price} x${item.quantity}`);
      });
    ...
    console.log('📊 Achievement: ' + ((totalSales / 8500) * 100).toFixed(1) + '%');
