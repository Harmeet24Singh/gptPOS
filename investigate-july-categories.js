...
async function investigateJulyCategories() {
  ...
  try {
    ...
    allJulyTransactions.forEach(transaction => {
      ...
      transaction.items.forEach(item => {
        if (!categoryBreakdown[item.category]) {
          categoryBreakdown[item.category] = { count: 0, value: 0, transactions: new Set() };
        }
        categoryBreakdown[item.category].count += item.quantity;
        categoryBreakdown[item.category].value += (item.total || item.price * item.quantity);
        categoryBreakdown[item.category].transactions.add(transaction.transactionId);
      });
    });
    ...
    Object.entries(categoryBreakdown).forEach(([category, data]) => {
      console.log(`${category}: $${data.value.toFixed(2)} (${data.count} items, ${data.transactions.size} transactions)`);
    });
    ...
    if (mixedTransactions.length > 0) {
      ...
      mixedTransactions.slice(0, 5).forEach(t => {
        console.log(`\nTransaction ${t.id}: ${t.categories.join(" + ")} - $${t.total.toFixed(2)}`);
    ...
    const alcoholOnlyValue = alcoholOnlyTransactions.reduce((sum, t) => sum + t.total, 0);
