...
async function generateTobaccoSalesSeptember() {
  ...
  try {
    ...
    console.log(
      "\n🚬 Generating September 2025 tobacco sales targeting $5,900..."
    );
    ...
    // Get starting transaction ID for September
    ...
    let currentTransactionId = 91001; // Start from clean range for September tobacco
    ...
    // Generate transactions throughout September 2025
    const daysInSeptember = 30;

    console.log(
      `📅 Generating tobacco transactions for September 1-30, 2025...`
    );
    ...
    while (totalGenerated < targetAmount) {
      ...
      if (transactions.length > 500) {
        console.log("⚠️  Reached 500 transactions, stopping generation");
        break;
      }
    }
    ...
    if (transactions.length > 0) {
      ...
      console.log(`\n✅ Verification:`);
      console.log(`   September tobacco transactions in DB: ${verifyCount}`);
      console.log(
        `   Total September tobacco sales: $${
          verifyTotal[0]?.total.toFixed(2) || 0
        }`
      );

      // Final summary with all categories for September
      ...
      console.log(`\n🎉 September 2025 tobacco sales generation complete!`);
    } else {
      console.log("❌ No transactions generated");
    }
  } catch (error) {
    console.error("Error generating September tobacco sales:", error);
  } finally {
    await client.close();
  }
}

generateTobaccoSalesSeptember();
