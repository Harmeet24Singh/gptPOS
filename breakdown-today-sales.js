...
async function calculateTodaysSalesBreakdown() {
  ...
  try {
    ...
    const todaysTransactions = await collection.find({
      timestamp: {
    ...
    todaysTransactions.forEach((transaction, index) => {
      ...
      if (total < 0) {
        ...
        console.log(`❌ Negative: $${total.toFixed(2)} - ${transaction.items?.[0]?.name || 'Unknown'}`);
      } else {
    ...
    console.log(`📉 Lotto Payouts: $${lottoPayouts.toFixed(2)} (${negativeTransactions.length} transactions)`);
    ...
    console.log(`   Net (positive - negative): $${(regularSales - lottoPayouts).toFixed(2)}`);
    console.log(`   Expected from UI: $110.96`);
    
    // Let's see if excluding the biggest negative transaction gives us closer to 110.96
    const transactionsExcludingBigPayout = todaysTransactions.filter(tx => tx.total !== -500);
    const totalExcludingBigPayout = transactionsExcludingBigPayout.reduce((sum, tx) => sum + (tx.total || 0), 0);
    console.log(`   Excluding -$500 payout: $${totalExcludingBigPayout.toFixed(2)}`);
