...
async function fixAllJanuaryPaymentMethods() {
    ...
    try {
        ...
        const problematicTransactions = await collection.find({
            timestamp: {
        ...
        console.log(`Found ${problematicTransactions.length} transactions needing payment method fixes`);
        ...
        for (const transaction of problematicTransactions) {
            ...
            if (!transaction.transactionId) {
                ...
                updateDoc.transactionId = lastTransaction ? lastTransaction.transactionId + 1 : 210000;
            }
        ...
        console.log(`   Cash transactions: ${finalBreakdown.cash} ($${finalBreakdown.totalCash.toFixed(2)})`);
        console.log(`   Card transactions: ${finalBreakdown.card} ($${finalBreakdown.totalCard.toFixed(2)})`);
        console.log(`   Missing payment methods: ${finalBreakdown.missing}`);
        console.log(`   Total sales: $${(finalBreakdown.totalCash + finalBreakdown.totalCard).toFixed(2)}`);
        
        const cashPercentage = (finalBreakdown.cash / (finalBreakdown.cash + finalBreakdown.card)) * 100;
        console.log(`   Payment split: ${cashPercentage.toFixed(1)}% cash, ${(100 - cashPercentage).toFixed(1)}% card`);
