...
async function fixJanuaryPaymentFields() {
  ...
  try {
    ...
    const januaryAlcohol = await collection
      .find({
        timestamp: {
    ...
    console.log(
      `Found ${januaryAlcohol.length} January alcohol transactions to fix`,
    );
    ...
    for (const transaction of januaryAlcohol) {
      ...
      if (transaction.paymentMethod === "cash") {
        ...
        if (
          transaction.cardAmount === undefined ||
    ...
    console.log(
      `✅ Updated ${updated} transactions with proper payment fields`,
    );
    ...
    console.log(
      `   Cash transactions: ${cashCount} ($${totalCashAmount.toFixed(2)})`,
    );
    console.log(
      `   Card transactions: ${cardCount} ($${totalCardAmount.toFixed(2)})`,
    );
    console.log(
      `   Total alcohol sales: $${(totalCashAmount + totalCardAmount).toFixed(2)}`,
    );
    ...
    samples.forEach((t, i) => {
      console.log(
        `   ${i + 1}. ID ${t.transactionId}: ${t.paymentMethod} - Cash: $${t.cashAmount}, Card: $${t.cardAmount}, Total: $${t.total}`,
      );
    });
  } catch (error) {
