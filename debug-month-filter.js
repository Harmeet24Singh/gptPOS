...
const testMonthFilter = (monthFilter) => {
    ...
    const monthStart = new Date(`${year}-${month.padStart(2, "0")}-01T00:00:00.000Z`);
    const nextMonth = parseInt(month) === 12 ? 1 : parseInt(month) + 1;
    const nextYear = parseInt(month) === 12 ? parseInt(year) + 1 : parseInt(year);
    const monthEnd = new Date(`${nextYear}-${nextMonth.toString().padStart(2, "0")}-01T00:00:00.000Z`);
    ...
    const localMonthStart = new Date(parseInt(year), parseInt(month) - 1, 1, 0, 0, 0, 0);
    const localNextMonth = parseInt(month) === 12 ? 1 : parseInt(month) + 1;
    const localNextYear = parseInt(month) === 12 ? parseInt(year) + 1 : parseInt(year);
    const localMonthEnd = new Date(localNextYear, localNextMonth - 1, 1, 0, 0, 0, 0);
    ...
    console.log("Is sample between UTC dates?", sampleTransactionDate >= monthStart && sampleTransactionDate < monthEnd);
    console.log("Is sample between local dates?", sampleTransactionDate >= localMonthStart && sampleTransactionDate < localMonthEnd);
};
