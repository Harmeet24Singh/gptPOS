...
async function generateOctoberLotterySales() {
  ...
  try {
    ...
    console.log(
      "\n🎰 Generating October 2025 lottery sales targeting $26,987..."
    );

    // Check if October lottery data already exists
    const existingOctoberLottery = await transactionCollection
      .find({
        timestamp: {
    ...
    if (existingOctoberLottery.length > 0) {
      console.log(
        `⚠️  Found ${existingOctoberLottery.length} existing October lottery transactions. Deleting first...`
      );
      ...
      console.log(
        `✅ Cleared existing October lottery data (${deleteResult.deletedCount} transactions)`
      );
    }

    // Get starting transaction ID for October lottery
    ...
    let currentTransactionId = 95001; // Start from clean range for October lottery
    ...
    console.log(`📅 Generating lottery transactions for October 1-31, 2025...`);

    // Generate transactions day by day
    for (let day = 1; day <= 31; day++) {
      const date = new Date(
        `2025-10-${day.toString().padStart(2, "0")}T00:00:00.000Z`
      );
      const dayOfWeek = date.getDay(); // 0 = Sunday, 6 = Saturday

      // Lottery sales patterns by day of week - adjusted for October (31 days)
      let dailyTransactionCount;
      if (dayOfWeek === 0) {
        // Sunday
        dailyTransactionCount = Math.floor(Math.random() * 18) + 32; // 32-50
      } else if (dayOfWeek === 6) {
        // Saturday
        dailyTransactionCount = Math.floor(Math.random() * 22) + 42; // 42-64
      } else if (dayOfWeek === 5) {
        // Friday
        dailyTransactionCount = Math.floor(Math.random() * 20) + 37; // 37-57
      } else {
        // Monday-Thursday
        dailyTransactionCount = Math.floor(Math.random() * 18) + 28; // 28-46
      }
      ...
      for (let i = 0; i < dailyTransactionCount; i++) {
        ...
        for (let j = 0; j < itemCount; j++) {
          // Favor higher-priced items to reach target faster
          let item;
          if (totalGenerated < targetAmount * 0.5) {
            // First half - mix of all items
            item =
              lotteryItems[Math.floor(Math.random() * lotteryItems.length)];
          } else {
            // Second half - favor higher-priced items
            const highValueItems = lotteryItems.filter((i) => i.price >= 5);
        ...
        if (rand < 0.35 && cardAmountGenerated + subtotal <= cardLimit) {
          paymentMethod = "card";
          cardAmount = subtotal;
          cashAmount = 0;
    ...
    console.log(`\n✅ Verification:`);
    console.log(`   October lottery transactions in DB: ${verifyCount}`);
    console.log(
      `   Total October lottery sales: $${
        verifyTotal[0]?.total.toFixed(2) || 0
      }`
    );

    console.log(`\n🎉 October 2025 lottery sales generation complete!`);
  } catch (error) {
    console.error("Error generating October lottery sales:", error);
  } finally {
    await client.close();
  }
}

generateOctoberLotterySales();
