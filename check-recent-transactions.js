  ...
  try {
    ...
    recentTransactions.forEach((transaction, index) => {
      ...
      if (Math.abs(transactionTotal - 110.96) < 0.01) {
        console.log(
          `   🎯 MATCH! This transaction totals $${transactionTotal.toFixed(2)} (expected $110.96)`,
        );
      }
    });
    ...
    const matchingTransactions = await collection
      .find({
        total: { $gte: 110.9, $lte: 111.0 },
      })
      .toArray();
    ...
    if (matchingTransactions.length > 0) {
      ...
      matchingTransactions.forEach((tx) => {
        console.log(
          `   ID: ${tx._id.toString().slice(-8)} - $${tx.total.toFixed(2)} on ${new Date(tx.timestamp).toLocaleString()}`,
        );
      });
    }
  } catch (error) {
