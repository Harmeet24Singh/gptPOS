...
async function checkJanuaryPaymentMethods() {
    ...
    try {
        ...
        const januaryAlcohol = await db.collection('transactions').find({
            timestamp: {
        ...
        januaryAlcohol.forEach((transaction, index) => {
            ...
            console.log(`   Payment Method: ${transaction.paymentMethod || 'MISSING'}`);
        ...
        const allJanAlcohol = await db.collection('transactions').find({
            timestamp: {
                $gte: new Date('2026-01-01'),
                $lt: new Date('2026-02-01')
            },
